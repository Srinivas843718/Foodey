...
const useMenuAPI=()=> {
    ...
    const handleUpdate=(e,index)=>{
        if(e.target.id=="noodles"){
            (dispatch({type:'ADD_FRONTEND_ITEM5',payload:{
                index
            }}))
        }
        if(e.target.id=="biryani"){
            (dispatch({type:'ADD_FRONTEND_ITEM4',payload:{
                index
            }}))
        }
        if(e.target.id=="pizza"){
            (dispatch({type:'ADD_FRONTEND_ITEM2',payload:{
                index
            }}))
        }
        if(e.target.id=="pasta"){
            (dispatch({type:'ADD_FRONTEND_ITEM3',payload:{
                index
            }}))
        }
        if(e.target.id=="burger"){
            (dispatch({type:'ADD_FRONTEND_ITEM1',payload:{
                index
            }}))
        }
        if(e.target.id=="dessert"){
            (dispatch({type:'ADD_FRONTEND_ITEM6',payload:{
                index
            }}))
        }
    }
